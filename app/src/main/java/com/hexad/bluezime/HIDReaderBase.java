        ...
        if (data[offset] == (byte) 0xa1 || data[offset] == (byte) 0xb1) {
            ...
            if (supportedReports.containsKey(data[offset + 1])) {
                ...
                if (neededBytes > remaining - 2) {
                    if (D)
                        Log.w(getDriverName(), "Got " + (remaining - 2) + " bytes for report " + data[offset + 1] + ", but need " + neededBytes);
                    return remaining;
                }
                ...
                if (D)
                    Log.w(getDriverName(), "Got an unsupported HID report: " + data[offset + 1] + ", length: " + (remaining - 2) + ", data: " + getHexString(data, offset, offset + (remaining)));
            }
        }
